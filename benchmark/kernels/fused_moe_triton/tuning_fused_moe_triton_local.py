...
def _remote_worker(_cls):
    return _cls if ray is None else ray.remote(num_gpus=1)(_cls)
...
class BenchmarkWorker:
    ...
    def __init__(
        ...
        self.device_id = (
            ...
            else (0 if is_hip() else int(ray.get_gpu_ids()[0]))
        )
...
def main(args: argparse.Namespace):
    ...
    if args.local:
        worker = BenchmarkWorker(
            args.seed, server_args, local_device_id=args.local_rank
        )
        ...
        if ray is None:
            raise RuntimeError("Ray is required unless --local is set")
        ray.init()
    ...
    if args.tune:
        ...
        tune_inputs = [(*values, search_space) for values in tuning_inputs]
        configs = _distribute("tune", tune_inputs)
        best_configs = {
            M: sort_config(config) for M, config in zip(batch_sizes, configs)
        }
        save_configs(
            best_configs,
            args.output or filename,
        )
        end = time.perf_counter()
        print(f"Tuning took {end - start:.2f} seconds")
    else:
        outputs = _distribute("benchmark", tuning_inputs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Run on the current process's visible GPU instead of Ray.",
    )
    parser.add_argument(
        "--local-rank",
        type=int,
        default=0,
        help="CUDA device ordinal used by --local.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Override the tuned config output path.",
    )
