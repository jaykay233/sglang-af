#!/usr/bin/env bash
# FP8 A2F wire dtype over StepMesh (scheme A).
set -euo pipefail
# shellcheck disable=SC1091
source "$(dirname "$0")/stepmesh_smoke_lib.sh"

export SGLANG_AFD_ROUTING_SCHEME=a
export SGLANG_AFD_A2F_DTYPE=fp8
LOG_DIR="${LOG_DIR:-/tmp/afd_stepmesh_fp8}"
ATTN_PORT="${ATTN_PORT:-32620}"
FFN_PORT="${FFN_PORT:-32621}"
PS_PORT="${PS_PORT:-18352}"
PIDS=()
afd_stepmesh_prep
...
echo "FP8 A2F FFN_GPU=$FFN_GPU ATTN_GPU=$ATTN_GPU"
(
...
rg -q "a2f_dtype=fp8|wire=torch.float8" "$LOG_DIR/attn.log" \
  || rg -q "a2f_dtype=fp8|float8" "$LOG_DIR/ffn.log" \
  || echo "WARNING: fp8 dtype not logged" >&2
...
afd_assert_sane_generation "$LOG_DIR/response.json" "AFD_STEPMESH_FP8_SMOKE_OK"
