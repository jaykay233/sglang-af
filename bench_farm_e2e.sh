...
# comma list: sticky,coalesce,spin_wait,continuous
...
if [[ -n "${MOE_BACKEND:-}" ]]; then
  COMMON_ARGS+=(--moe-runner-backend "$MOE_BACKEND")
fi
...
apply_mode_env() {
  ...
  case "$mode" in
    ...
    spin_wait)
      ...
      ;;
    continuous)
      export SGLANG_AFD_FARM_COALESCE_K="${CONTINUOUS_COALESCE_K:-1}"
      export SGLANG_AFD_FARM_MAX_INFLIGHT="${CONTINUOUS_MAX_INFLIGHT:-8}"
      export SGLANG_AFD_FARM_MAX_INFLIGHT_PER_LAYER="${CONTINUOUS_PER_LAYER_CAP:-1}"
      export SGLANG_AFD_FARM_SCHED="${CONTINUOUS_SCHED:-oldest}"
      export SGLANG_AFD_NUM_MB="${SGLANG_AFD_FARM_MAX_INFLIGHT}"
      export SGLANG_AFD_ATTN_QUEUE_ENABLE=1
      export SGLANG_AFD_ATTN_QUEUE_BYPASS_FIRST_LAYER=0
      export SGLANG_AFD_ATTN_QUEUE_TARGET_TOKENS="${CONTINUOUS_ATTN_TARGET_TOKENS:-64}"
      export SGLANG_AFD_ATTN_QUEUE_MAX_HOPS="${CONTINUOUS_ATTN_MAX_HOPS:-4}"
      export SGLANG_AFD_FFN_QUEUE_ENABLE=1
      export SGLANG_AFD_FFN_QUEUE_TARGET_TOKENS="${CONTINUOUS_FFN_TARGET_TOKENS:-64}"
      export SGLANG_AFD_FFN_QUEUE_MAX_HOPS="${CONTINUOUS_FFN_MAX_HOPS:-4}"
      export SGLANG_AFD_FFN_QUEUE_MAX_GLOBAL_HOPS="${CONTINUOUS_FFN_MAX_GLOBAL_HOPS:-32}"
      ;;
    *)
...
trap 'cleanup_mode sticky; cleanup_mode coalesce; cleanup_mode spin_wait; cleanup_mode continuous' EXIT
